...
@push('footer')
    ...
    <link rel="stylesheet" href="{{ asset('loding/paymentDGP.css') }}">
    <div class="DGPAY">
        <div class="pay-msg">Vous avez lancé un paiement veuillez le finaliser sur la page de l'operateur svp !</div>
        <img src="{{ asset('loding/loader.gif') }}" alt="loding">
        <div onclick="fermer()" class="fermer ">
            <div>
                <div><a class="btn bg-orange">{{ __('Close') }}</a></div>
            </div>
        </div>
        <div onclick="terminer()" class="terminer">
            <div>
                <div><a class="btn bg-orange">{{ __('Finish') }}</a></div>
            </div>
        </div>
    </div>
    @php
        $montant = $promotionDiscountAmount + $couponDiscountAmount - $shippingAmount > Cart::instance('cart')->rawTotal() ? 0 : format_price(Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount, null, true);
    @endphp
    <script>
        $('#payment_cod').prop( "checked", true );
        var montant = {{ $montant }}
        var token = "TIC6TQXVDIG22"
        var ssToken = "{{ uniqid() }}"
        var params =
            'scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,width=800,height=600,left=auto,top=auto';

        function boxpay() {
            $('.DGPAY').addClass('active-message')

            if ($("#payment_bank_transfer").is(":checked")) {
                let open_pay = open('https://pay.digital.ci/formulaire/paiement?amount=' + montant + '&token=' + token +
                    '&ssToken=' + ssToken, 'DigitalPAy', params);
                reload = setInterval(() => {
                    $.ajax({
                        method: 'GET',
                        url: "https://pay.digital.ci/transaction/status/" + montant + "/" + ssToken,
                        success: function(data) {
                            $('.DGPAY').addClass('active-message')
                            data = JSON.parse(data)
                            if (data.status == 'FAILED') {
                                $('.pay-msg').html(
                                    'Paiement echoué , veuillez rééssayer svp !'
                                )
                                $('.fermer').addClass('mgs-pay')
                                open_pay.close();
                            }
                            if (data.status == 'SUCCES') {
                                clearInterval(reload)
                                $('.pay-msg').html(
                                    'Paiement réussit , veuillez finaliser la commande svp !'
                                )
                                $('.terminer').addClass('mgs-pay')
                                open_pay.close();
                            }
                        },
                    })
        ...
        function fermer() {
            ...
            let close_pay = open('https://pay.digital.ci/sortir/paiement/' + ssToken, 'DigitalPAy', params);
            setTimeout(() => {
                close_pay.close();
                location.reload()
            }, 2000);
        ...
        function terminer() {
            ...
            setTimeout(() => {
                close_pay.close();
                $('.DGPAY').removeClass('active-message')
                $('.fermer').addClass('mgs-pay')
            }, 2000);
        }
    </script>
